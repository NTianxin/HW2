...
function begin_play(){
		...
		if(!isEmpty(player1) && !isEmpty(player2)){
				...
				document.getElementById('turn_info').innerHTML = "Turn for: " + document.getElementById("player1_id").value.bold();
			 	started = true;
		} else {
						...
						if(whose_move()){
							if(board_state[indexInput] == -1){
								...
								document.getElementById("turn_info").innerHTML = "Turn for: <strong> O </strong>";
									...
									alert("Position already filled! Please try again");
							}
						}
						else {
							if(board_state[indexInput] == -1){
								board_state[indexInput] = 0;
...
//helper function to output the Winner
